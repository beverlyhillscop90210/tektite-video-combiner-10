...
class TektiteVideoCombiner10:
    ...
    def combine_videos(
        ...
        **kwargs,
        ...
        while time.time() < deadline:
            ...
            raise ValueError(
                f"Tektite Video Combiner 10.0 timed out after {wait_timeout_sec}s while waiting. Last state: {last_state}"
            )
        print(
            f"[Tektite Video Combiner 10.0] Ready {expected_clips}/{expected_clips}. Starting normalize+stitch."
        )
        ...
        try:
            ...
            for idx, unit in enumerate(ordered_units, start=1):
                if unit["kind"] == "video":
                    print(
                        f"[Tektite Video Combiner 10.0] Clip {idx}/{len(ordered_units)} is video: "
                    ...
                    print(
                        f"[Tektite Video Combiner 10.0] Clip {idx}/{len(ordered_units)} is image sequence: "
                    ...
                    print(
                        f"[Tektite Video Combiner 10.0] Clip {idx}/{len(ordered_units)} sequence render done: "
                        f"{seq_video_path}"
                    )
    ...
    def _build_progress_line(
        ...
        *,
        ...
        return (
            f"[Tektite Video Combiner 10.0] Clips ready: {ready_clips}/{expected_clips} | "
    ...
    def _persist_image_tensor_sequence(self, tensor: torch.Tensor, *, slot_index: int) -> List[str]:
        ...
        paths: List[str] = []
        print(
            f"[Tektite Video Combiner 10.0] clip{slot_index}: received IMAGE batch with "
    ...
    def _extract_paths_from_value(self, value: Any, _depth: int = 0, _seen: Optional[set] = None) -> List[str]:
        ...
        if hasattr(value, "get_stream_source"):
            try:
                ...
                print(f"[Tektite Video Combiner 10.0] get_stream_source failed: {type(exc).__name__}: {exc}")
        ...
        if hasattr(value, "save_to"):
            try:
                ...
                print(f"[Tektite Video Combiner 10.0] save_to fallback failed: {type(exc).__name__}: {exc}")
    ...
    def _log_unrecognized_clip_input(self, slot_index: int, value: Any) -> None:
        ...
        print(
            f"[Tektite Video Combiner 10.0] clip{slot_index}: input connected but not recognized "
    ...
    def _render_image_sequence_to_video(
        ...
        *,
        ...
        print(
            f"[Tektite Video Combiner 10.0] Rendering image sequence: "
        ...
        print(f"[Tektite Video Combiner 10.0] Image sequence video ready: {out_path}")
        return out_path
    ...
    def _ensure_writable_output_path(self, resolved_output: str) -> str:
        ...
        print(
            f"[Tektite Video Combiner 10.0] Output directory not writable: {out_dir}. "
...
NODE_CLASS_MAPPINGS = {
    "TektiteVideoCombiner10": TektiteVideoCombiner10,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "TektiteVideoCombiner10": "Tektite Video Combiner 10.0",
}
